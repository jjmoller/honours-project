...
group_by(ID) %>%
slice_max(gamma, n = 1) %>%
ungroup() %>% filter(topic==7) %>% arrange(desc(gamma))
...
data_topics_with_sentiment %>% 
  ...
  ungroup() %>% 
  group_by(Outlet_Name) %>% 
  mutate(ave_topic_sentiment= ave(topic_sentiment)) %>% slice(Outlet_Name, n=1) %>%
  select("Outlet_Name", "ave_topic_sentiment"), by="Outlet_Name") %>% 
  arrange(desc(n))
