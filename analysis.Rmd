...
contributors: "Juan Möller", "Daniel Small", "Gabrielle Joubert"
...
initial_clean_data %>% 
  ...
  geom_vline(xintercept = as.numeric(as.Date("2020-03-11")), linetype=4, color="#336666")+
  geom_text(aes(as.Date("2020-03-10"),13, label = "2020-03-11", vjust = -2))+
  geom_hline(yintercept=4.396, linetype=4)+
  geom_line(size=1, colour="black") +
  scale_x_date(labels = date_format("%m-%Y"))+
  labs(x = "Date", y = "Number of published articles",
       title = "")+
  theme_classic(base_size = 20)
ggsave("article_numbers_over_time.png", width=10,height = 5, dpi=500)
```
...
articles_title_with_sentiment <- initial_clean_data %>%
  ...
  sentiment_by(.$ID, 
               ...
               question.weight = 0,
               neutral.nonverb.like = TRUE)
articles_title_with_sentiment %>% add_column(type="title") %>% 
  rbind(articles_with_sentiment%>% add_column(type="body")) %>% 
  ggplot(aes(x=ave_sentiment, color=type))+
  geom_density(size=1) + 
  geom_vline(xintercept=0, linetype=4)+
  labs(x = "Average sentiment", y = "Density", color="")+
  theme_classic(base_size = 20)
  #theme(legend.position="bottom")
#ggsave("article_body_vs_title_sentiment.png", width=10, dpi=500)
```
...
Top 5 positive and negative outlets by published article sentiment
```{r}
data_with_sentiment_summary %>% 
  filter(n>4) %>% 
arrange(desc(mean)) %>%
    slice(1:5) %>% 
 rbind(data_with_sentiment_summary %>% 
  filter(n>4) %>% 
    arrange(mean)%>%
    slice(1:5))
```
...
p1<-data_with_sentiment %>% 
 ...
 ggplot(aes(x = fct_reorder(website_category, ave_sentiment), y = ave_sentiment)) +
  ...
  geom_point(aes(y = ave_sentiment), 
             ...
             size = 1, alpha = 0.5) + 
  guides(fill = "none") +
  labs(x = "", y = "Sentiment", title = "", subtitle = "Website categories")+
  coord_flip()+
...
p2<-data_with_sentiment %>% 
  ...
  add_count(Outlet_Name) %>% 
  filter(n > 4) %>%  #Only show outlets with more than 4 published articles
  ...
  geom_point(aes(y = ave_sentiment), 
             ...
             size = 1, alpha = 0.5) + 
  guides(fill = "none") +
  labs(x = "", y = "Sentiment", title = "", subtitle = "Popular news media outlets")+
  coord_flip()+
  theme_classic(base_size=15) +
  theme(
    panel.grid.major.x = element_blank(),
...
ggsave("sentiment_boxplot_with_zoom.png", width=15, height=8, dpi=500)
```
...
data_with_sentiment %>% select(website_category, ave_sentiment, week) %>% 
  ...
  geom_vline(xintercept = as.numeric(as.Date("2020-03-11")), linetype=4, color="black")+
  geom_text(aes(as.Date("2020-03-10"),0, label = "2020-03-11",vjust=-39))+
  geom_hline(yintercept = 0.090, linetype=4)+
  #geom_line() +
  geom_link2(aes(colour = after_stat(ifelse(y > 0, "positve", "negative"))),size=1)+
  scale_x_date(labels = date_format("%m-%Y"))+
  ...
  theme(legend.position="bottom")
ggsave("Mean_sentiment_per_week.png", width=14, height=8, dpi=500)
```
...
#Article body vs article title sentiment comparison
```{r}
body_vs_title_sentiment<-articles_with_sentiment %>% 
  inner_join(articles_title_with_sentiment, by="by") %>%  #should we filter out headlines where sentiment=0?
  mutate(diff=-1*(ave_sentiment.x-ave_sentiment.y)) %>% 
   filter(ave_sentiment.x>=0 & ave_sentiment.y>=0) %>% rbind(articles_with_sentiment %>% 
  inner_join(articles_title_with_sentiment, by="by") %>% 
  mutate(diff=-1*(ave_sentiment.x-ave_sentiment.y)) %>% 
  filter(ave_sentiment.x<0 & ave_sentiment.y<0)) %>% rbind(articles_with_sentiment %>% 
  inner_join(articles_title_with_sentiment, by="by") %>% 
  mutate(diff=-1*(ave_sentiment.x-ave_sentiment.y) )%>% 
  filter(ave_sentiment.x>=0 & ave_sentiment.y<0))%>% rbind(articles_with_sentiment %>% 
  inner_join(articles_title_with_sentiment, by="by") %>% 
  mutate(diff=ave_sentiment.y-ave_sentiment.x) %>% 
  filter(ave_sentiment.x<0 & ave_sentiment.y>=0)) %>% 
  rename(ID = by) %>% 
  arrange(ID)
body_vs_title_sentiment
summarise(body_vs_title_sentiment, Average = mean(diff, na.rm = T))
```

#Descriptive statistics: Sentiment difference by website category
```{r}
body_vs_title_sentiment %>% left_join(data_with_sentiment, by="ID") %>% 
  group_by(website_category) %>% 
    summarise(ave_body_sen = mean(ave_sentiment.x),
              body_median = median(ave_sentiment.x),
              body_sd = sd(ave_sentiment.x),
              ave_title_sen = mean(ave_sentiment.y),
              title_median = median(ave_sentiment.y),
              title_sd = sd(ave_sentiment.y),
              ave_diff = mean(diff)) %>% 
  arrange(ave_diff)
...
data_topics_with_sentiment %>% 
  ...
  # group_by(topic) %>% 
  # #mutate(topic = as.factor(topic)) %>% 
  # mutate(ave_topic_sentiment=ave(topic_sentiment)) %>% 
  # ungroup() %>% 
  # group_by(topic) %>% 
  # distinct(topic, .keep_all= TRUE) %>% 
  # select(topic, ave_topic_sentiment) %>% 
  # arrange(ave_topic_sentiment)
  ggplot(aes(x = fct_reorder(topic, topic_sentiment, .fun = median), y = topic_sentiment)) +
  geom_hline(yintercept=0, linetype=4)+
  geom_boxplot(fill="#9DE4E7") +
  coord_flip() +
  labs(x = "Topic", y = "Sentiment", title = "Topic sentiment values", subtitle = "")+
  theme_classic(base_size = 15)+
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank())
#ggsave("topics_with_sentiment.png", width=10, dpi=500)
```
...
data_topics_with_labels$topic[data_topics_with_labels$topic == "1"] <- "Technology skill development"
data_topics_with_labels$topic[data_topics_with_labels$topic == "2"] <- "Increased screen time due to remote learning"
data_topics_with_labels$topic[data_topics_with_labels$topic == "3"] <- "Parental concerns"
data_topics_with_labels$topic[data_topics_with_labels$topic == "4"] <- "Support and guidance for parents"
data_topics_with_labels$topic[data_topics_with_labels$topic == "5"] <- "Gaming concerns"
data_topics_with_labels$topic[data_topics_with_labels$topic == "6"] <- "Perspectives on coping with screen time"
data_topics_with_labels$topic[data_topics_with_labels$topic == "7"] <- "Regulatory applications"
data_topics_with_labels$topic[data_topics_with_labels$topic == "8"] <- "Eye health"
data_topics_with_labels$topic[data_topics_with_labels$topic == "9"] <- "Social media and mental health in adolescents"
data_topics_with_labels$topic[data_topics_with_labels$topic == "10"] <- "Mobile screentime effects"
data_topics_with_labels$topic[data_topics_with_labels$topic == "11"] <- "Sedentary lifestyle"
data_topics_with_labels$topic[data_topics_with_labels$topic == "12"] <- "Impact of screen time on literacy"
data_topics_with_labels$topic[data_topics_with_labels$topic == "13"] <- "Communicating research findings with the public"
data_topics_with_labels$topic[data_topics_with_labels$topic == "14"] <- "Cognitive development and sleep"
data_topics_with_labels$topic[data_topics_with_labels$topic == "15"] <- "Government's role in screen time policy"
p3<-data_topics_with_labels %>%
arrange(topic) %>% 
mutate(topic = as.factor(topic)) %>%
ggplot(aes(x = topic, y = topic_sentiment, fill=website_category)) + #change to outlet category
geom_hline(yintercept=0, linetype=4)+
geom_boxplot() +
coord_flip() +
labs(x = "", y = "Sentiment", subtitle = "Website Category")+
theme_classic(base_size = 20)+
theme(panel.grid.major.x = element_blank(),
...
p3
#ggsave("sentiment_per_website_category.pdf", width=10,height = 6, dpi=500)
```
...
add_count(Outlet_Name) %>%
filter(n>28) %>%
ggplot(aes(x = topic, y = topic_sentiment, fill=Outlet_Name)) + #change to outlet category
geom_hline(yintercept=0, linetype=4)+
geom_boxplot() +
coord_flip() +
labs(x = "", y = "Sentiment", subtitle = "Popular News Media")+
theme_classic(base_size = 20)+
theme(panel.grid.major.x = element_blank(),
legend.position = "bottom",
legend.title = element_blank(),
...
p3p4<-p3+p4
#ggsave("sentiment_per_topic_patchwork.png", width=20, height=15, dpi=500)
p3p4
```
...
```{r}
data_topics_with_labels%>% 
  group_by(ID) %>% 
  slice_max(gamma,n=1) %>% #or should it be top 2 terms
  ggplot(mapping = aes(as.Date(week))) +
   geom_vline(xintercept = (as.Date("2020-03-11")), linetype=4)+
  geom_bar(aes(fill= factor(topic)))+  #can also use geom_point
  labs(x = "week", y = "count", subtitle = "All website categories")+
  theme_classic(base_size = 12)+
  theme(legend.position = "none")+
  theme(axis.text.x = element_text(angle = 45, vjust=0.5))+
  facet_wrap(~topic, ncol=5, labeller = labeller(topic = label_wrap_gen(25)))
ggsave("prevailing_article_topics_over_time.png", width=10,height = 6, dpi=500)
```
...
```{r}
outlet_topic_count <- data_with_gamma_with_sentence_sentiment_count %>%
group_by(topic) %>%
# split(initial_clean_data$Date_Published<as.Date("2020-03-11")) %>%
tally(sort=TRUE) %>%
mutate(freq = round((n / sum(n)) * 100,2))
...
```{r}
data_topics_with_sentiment %>%
select("ID","topic","gamma","normalised_gamma","ave_sentiment","Author","Outlet_Name", "Title","topic_sentiment") %>%
group_by(ID) %>%
slice_max(gamma, n = 1) %>%
ungroup() %>% filter(topic==13) %>% arrange(desc(gamma))
```

#Who reports on the popular topics and how do they report on 

Topic 15
```{r}
#Select all articles where this topic has the highest gamma score. Then get the average topic sentiment for this topic per outlet. 
data_topics_with_sentiment %>% 
  group_by(ID) %>% 
  slice_max(gamma, n = 1) %>%
  ...
  filter(topic==3) %>% 
  group_by(Outlet_Name) %>% 
  summarise(ave_topic_sentiment= ave(topic_sentiment)) %>%
  left_join(data_topics_with_sentiment %>% 
            group_by(ID) %>% 
            slice_max(gamma, n = 1) %>%
  filter(topic==3) %>% 
  group_by(Outlet_Name) %>% 
  summarise(n=n()), by="Outlet_Name") %>% 
  ungroup() %>% 
  distinct(Outlet_Name, .keep_all= TRUE) %>% 
  arrange(desc(n))
```
```{r}
#Top and least reported on topics by website category - before and after WHO announcement
data_topics_with_sentiment %>% 
  filter(Date_Published>as.Date("2020-03-11")) %>% 
  group_by(ID) %>% 
  slice_max(gamma, n = 1) %>%
  select("ID","topic","gamma","ave_sentiment","Author","website_category", "topic_sentiment") %>% 
  filter(website_category=="Tech news media") %>% 
  group_by(topic) %>% 
  tally() %>% 
  mutate(freq = round((n / sum(n)) * 100,2)) %>% 
  arrange(desc(freq)) %>%
...
#   slice_max(gamma, n = 1) %>%
#   select("ID","topic","gamma","ave_sentiment","Author","website_category", "topic_sentiment") %>% 
#   filter(website_category=="Business news media") %>% 
#   group_by(topic) %>% 
#   tally() %>% 
#   mutate(freq = round((n / sum(n)) * 100,2)) %>% 
#   arrange(freq) %>%
...
#Mean sentiment (and SD) of weekly mean sentiment
```{r}
data_with_sentiment %>% select(website_category, ave_sentiment, week) %>% 
  group_by(week) %>%  
  summarise(mean_sentiment = mean(ave_sentiment)) %>%
  select(mean_sentiment) %>% 
  summarise(m_sentiment = mean(mean_sentiment),
...
#Getting Article Period (Before vs After) + T_test for overall before vs after
```{r}
data_before <- data_with_sentiment %>% select(website_category, ave_sentiment, Date_Published, week, sd, ID) %>% 
  filter(Date_Published < as.Date("2020-03-11")) %>%
  mutate(period = "Before")

data_after <- data_with_sentiment %>% select(website_category, ave_sentiment, Date_Published, week, sd, ID) %>% 
  filter(Date_Published > as.Date("2020-03-11")) %>%
  mutate(period = "After")

data_split <- data_before %>%
  rbind(data_after) %>%
  t_test(ave_sentiment ~period)
data_split
```

#Getting Article Period (Before vs After) + Cohens_d for overall before vs after
```{r}
data_before <- data_with_sentiment %>% select(website_category, ave_sentiment, Date_Published, week, sd, ID) %>% 
  filter(Date_Published < as.Date("2020-03-11")) %>%
  mutate(period = "Before")
...
data_split <- data_before %>%
  rbind(data_after) %>%
  cohens_d(ave_sentiment ~period)
data_split
```

#T-test for titles vs body
```{r}
body_vs_title_sentiment <- articles_title_with_sentiment %>% add_column(type="title") %>% 
  rbind(articles_with_sentiment%>% add_column(type="body")) %>%
  t_test(ave_sentiment ~ type)
body_vs_title_sentiment
```

#Cohens_d for title vs body
```{r}
body_vs_title_sentiment_cohens_d <- articles_title_with_sentiment %>% add_column(type="title") %>% 
  rbind(articles_with_sentiment%>% add_column(type="body")) %>%
  cohens_d(ave_sentiment ~ type)
body_vs_title_sentiment_cohens_d
```

#T-test for titles before vs after
```{r}
title_before_vs_after_sentiment <- articles_with_sentiment %>%
  inner_join(articles_title_with_sentiment, by="by") %>%
  rename(ID = by) %>%
  inner_join(data_with_sentiment, by = "ID") %>%
  select(ave_sentiment.y, Date_Published, week, sd, ID, website_category)

data_title_before <- title_before_vs_after_sentiment %>% select(website_category, ave_sentiment.y, Date_Published, week, sd, ID) %>% 
  filter(Date_Published < as.Date("2020-03-11")) %>%
  mutate(period = "Before")

data_title_after <- title_before_vs_after_sentiment %>% select(website_category, ave_sentiment.y, Date_Published, week, sd, ID) %>% 
  filter(Date_Published > as.Date("2020-03-11")) %>%
  mutate(period = "After")
 
data_title_split <- data_title_before %>%
  rbind(data_title_after) %>%
  t_test(ave_sentiment.y ~period)
data_title_split
```

#Cohens_d for titles before vs after
```{r}
title_before_vs_after_sentiment <- articles_with_sentiment %>%
  inner_join(articles_title_with_sentiment, by="by") %>%
  rename(ID = by) %>%
...
data_title_split_cohens <- data_title_before %>%
  rbind(data_title_after) %>%
  cohens_d(ave_sentiment.y ~period)
data_title_split_cohens
```
#t-test for topics before vs after WHO
```{r}
data_topics_with_labels_split<-data_topics_with_sentiment %>% 
   group_by(ID) %>% 
   slice_max(gamma,n=1) %>% 
   ungroup %>% 
   filter(topic=="Gaming concerns")

data_topics_with_labels_split_before <- data_topics_with_labels_split %>% 
  filter(Date_Published < as.Date("2020-03-11")) %>% 
  mutate(period = "Before")

data_topics_with_labels_split_after <- data_topics_with_labels_split %>% 
  filter(Date_Published > as.Date("2020-03-11")) %>% 
  mutate(period = "After")

data_topics_with_labels_split_before_and_after<-data_topics_with_labels_split_before %>%
  rbind(data_topics_with_labels_split_after)

data_topics_with_labels_split_before_and_after %>% select(topic, topic_sentiment, period) %>% 
  t_test(topic_sentiment ~ period)
